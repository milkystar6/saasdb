...
#               4.开启MySQL数据库监控
#                     已完成: node_exporter安装
#                     TODO : 1、pmm规范自动化安装 2、持久化端口和数据库实例的对应关系(更新到saasdb即可)
#    exit code: 0   正常退出脚本
#               100 数据库端口已占用
#               101 数据库启动失败
#               102 目标端口被占用且无可用端口
...
function parseopt() {
	...
	TEMP=$(getopt -o v:p:m:M:i:P:d:s:S:H: --long version:,port:,max_memory:,max_connections:,hub_ip:,hub_port:,hub_dir:,hub_ssh_user:,hub_ssh_user_passwd:,host_id: -- "$@")
	if [ $? -ne 0 ]; then
		echo "Terminating..." >&2
		print_usage
		on_failure 1
...
function init_mysql() {
	...
	conn_local_mysql="${local_base_dir}/$version/bin/mysql -uroot -p${default_root_passwd} -S ${local_data_dir}/data/sock/mysql.sock  "
...
function download_monitor_package() {
	#判断本地是否含有monitor文件
	yum install -y expect
	if [ -d "${local_monitor_dir}/pmm2" ]; then
		echo "目录存在"
	else
		echo "目录不存在"
		expect <<EOF
           set timeout 1200;
           spawn scp -P${hub_port} -r -p ${hub_ssh_user}@${hub_ip}:"${remote_monitor_File}/pmm2" "${local_monitor_dir}"
           expect {
             "*yes/no*" {send "yes\n";exp_continue}
             "*Permission denied*" {exit 1}
             "*password*" {send "${hub_ssh_user_passwd}\n";exp_continue}
...
function run_exporter() {
	...
	# TODO 如何持久化这部分内容
	# TODO 如何保证如果有节点故障，新来的exporter不会占用这个断端口
}

function check_process_exists() {
	if pidof -x "进程名" >/dev/null; then
		echo 0
	else
		echo 1
	fi
...
function add_monitor() {
	remote_monitor_File=${hub_dir}/monitor/mysql_monitor
...
function conn_saasdb() {
	...
	saasdb_admin_passwd=$(cat ${create_default_user_sql} | grep saasdb_admin | grep "CREATE USER" |
		...
		awk '{print $1}')
	conn2saasdb="${local_base_dir}/$version/bin/mysql -usaasdb_admin -p${saasdb_admin_passwd} saasdb"
}
