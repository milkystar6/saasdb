...
func (server *VariablesTaskClientService) ShowVariables(arr []*grpc_pb.ShowVariablesUseArray) (res *grpc_pb.HandleVariablesResponse, err error) {
...
// 精确匹配参数
func (server *VariablesTaskClientService) GetVariableRunningValue(variable string) (string, error) {
...
// 模糊匹配参数
func (server *VariablesTaskClientService) GetVariableRunningValueFuzzyMatching(variable string) (string, error) {
	// use gorm to get value and defer close session
	db, err := model.GormMysql(config.LoadConfig.MySQLManager.MysqlManagerUser, config.LoadConfig.MySQLManager.MysqlManagerPassword, config.LoadConfig.MySQLManager.MySQLManagerHost, "information_schema", config.LoadConfig.MySQLManager.MySQLManagerPort)
	if err != nil {
		return "", err
...
// 修改参数 每次只支持单个参数修改
func (server *VariablesTaskClientService) SetVariables(setvariablemap map[string]*grpc_pb.SetVariablesUseMap) (bool, error) {
	// use gorm to get value and defer close session
	db, err := model.GormMysql(config.LoadConfig.MySQLManager.MysqlManagerUser, config.LoadConfig.MySQLManager.MysqlManagerPassword, config.LoadConfig.MySQLManager.MySQLManagerHost, "information_schema", config.LoadConfig.MySQLManager.MySQLManagerPort)
	if err != nil {
		return false, err
