...
func (receiver *Instance) CreateInstance(db *gorm.DB) error {
	...
	sql := "REPLACE INTO `saasdb`.`saas_instance` (`created_at`,`updated_at`,`instance_name`,`host_id`,`domain_id`,`ip`,`port`,`application`,`version`,`use_type`,`health`,`role`,`feature`) " +
		"VALUES " +
		fmt.Sprintf("(NOW(),NOW(),'%v','%v','%v','%v','%v','%v','%v','%v','%v','%v','%v')", receiver.InstanceName, *receiver.HostId,*receiver.DomainId, receiver.Ip, *receiver.Port, receiver.Application, receiver.Version, receiver.UseType, receiver.Health, receiver.Role, f)
