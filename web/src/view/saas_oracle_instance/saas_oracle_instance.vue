<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="db_id" prop="dbId" width="120" />
        <el-table-column align="left" label="sid" prop="sid" width="120" />
        <el-table-column align="left" label="host_id" prop="hostId" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateSaasOracleInstanceFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteSaasOracleInstance,
  deleteSaasOracleInstanceByIds,
  updateSaasOracleInstance,
  findSaasOracleInstance,
  getSaasOracleInstanceList
...
const getTableData = async() => {
  const table = await getSaasOracleInstanceList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteSaasOracleInstanceFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteSaasOracleInstanceByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSaasOracleInstanceFunc = async(row) => {
    const res = await findSaasOracleInstance({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSaasOracleInstanceFunc = async (row) => {
    const res = await deleteSaasOracleInstance({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
